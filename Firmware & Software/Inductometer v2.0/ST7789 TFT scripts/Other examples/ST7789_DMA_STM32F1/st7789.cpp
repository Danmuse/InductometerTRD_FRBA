...
static void ST7789_WriteCommand(uint8_t cmd)
{
  ST7789_Select();
  ST7789_DC_Clr();
  // SPI.beginTransaction(SPISettings (2000000, MSBFIRST, SPI_MODE0, DATA_SIZE_16BIT));  // 2 MHz clock, MSB first, mode 0
...
static void ST7789_WriteData(uint8_t *buff, size_t buff_size)
{
  ...
  while (buff_size > 0) {
    ...
    {
      SPI.dmaSend(buff, chunk_size, 0);
      // HAL_SPI_Transmit_DMA(&ST7789_SPI_PORT, buff, chunk_size);
      // while (ST7789_SPI_PORT.hdmatx->State != HAL_DMA_STATE_READY) {}
    }
    else
      SPI.write(buff, chunk_size);
...
void ST7789_Init(void)
{
  ...
  SPI.beginTransaction(SPISettings (36000000, MSBFIRST, SPI_MODE3, DATA_SIZE_8BIT/*SPI_MODE0, DATA_SIZE_16BIT*/));  // 2 MHz clock, MSB first, mode 0
